...
export class UploadComponent implements OnInit {
    ...
    public uploadImage() {
        ...
        if (this.data.image || this.file.file_optional) {
            ...
            this._uploadService.makeFileRequest(this.URL, this.data.image,
                this.AUTH, this.file.root, this.formParams, this.file.method, this.file.rootTag,
                this.file.file_optional).subscribe((res) => {
                ...
                this.file.formParams.forEach((selInput) => {
                    this.formParams[selInput.title] = '';
                });
    ...
    public fileChangeListener($event) {
        ...
        if ((this.tagetFile.type &&
            this.file.file_format_list.indexOf(this.tagetFile.type) === -1)) {
    ...
    public onSelectFile() {
        if (this.uploader.queue && this.uploader.queue.length > 1) {
            this.uploader.queue = this.uploader.queue.splice(1);
        }
        // this.uploader.clearQueue();
        this.progress$.next(0);
        this.successFlag$.next(false);
    ...
    public uploadAll() {
        ...
        if (this.file.mode === 'video') {
            let that = this;
            let vid = document.createElement('video');
            let fileURL = URL.createObjectURL(this.uploader.queue[0]._file);
            vid.src = fileURL;
    ...
    public ngOnInit(): void {
        ...
        this.URL = ConfigService.getAPI() + 'jobseekers/' + this.userId + '/'
            + this.file.post_url + '/' + this.file.id + '/upload_document';

        if (this.file.mode === 'resume' || this.file.mode === 'coverletter' ||
            this.file.mode === 'video') {
            this.URL = ConfigService.getAPI() +
                'jobseekers/' + this.userId + '/' + this.file.post_url;
        }

        if (this.file.mode === 'company_cover') {
            this.URL = ConfigService.getAPI() + 'companies/' +
                this.authService.getCompanyId() + '/upload_cover';
        }

        if (this.file.mode === 'video' || this.file.mode === 'profile') {
            if (this.authService.getCheckEmployer()) {
                this.URL = ConfigService.getAPI() + 'companies/' +
                    this.authService.getCompanyId() + '/upload_avatar';
            } else {
                this.URL = ConfigService.getAPI() + 'users/' +
                    this.userId + '/' + this.file.post_url;
        ...
        if (this.file.mode === 'profile_member') {
            if (this.authService.getCheckEmployer()) {
                this.URL = ConfigService.getAPI() + 'companies/' +
                    this.authService.getCompanyId() + '/company_members/' + this.file.selId;
            }
        }
        if (this.file.mode === 'company_culture') {
            ...
            if (this.authService.getCheckEmployer()) {
                this.URL = ConfigService.getAPI() + 'companies/' +
                    this.authService.getCompanyId() + '/cultures/' + this.file.selId;
            }
        }
        ...
        this.uploader.onBuildItemForm = (fileItem, form) => {
            ...
            if (this.file.file_default && this.file.mode === 'coverletter') {
                form.append('jobseeker_coverletter[default]',
                    fileItem._file, this.file.file_default);
            }
            if (this.file.file_default && this.file.mode === 'resume') {
                form.append('jobseeker_resume[default]',
                    fileItem._file, this.file.file_default);
            }
            if (this.file.mode === 'company_culture') {
                ...
                this.file.formParams.forEach((selInput) => {
                    if (this.formParams[selInput.title]) {
                        form.append('culture[' + selInput.title + ']',
                            this.formParams[selInput.title]);
                    }
