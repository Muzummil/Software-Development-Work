...
@Component({
    ...
    styleUrls: ['./companyGeneralMobile.scss']
...
export class CompanyGeneralMobileComponent implements OnInit {
    ...
    public locationOffice$: BehaviorSubject<any> = new BehaviorSubject({});
    public companyObj;
    ...
    public getCoOrdinates(mode = 3) {
        ...
        if (mode == 3) {
            addressUrl = this.commonForm.controls['address_line1'].value + '+' +
                ...
                this.commonForm.controls['po_box'].value + '+' +
                this.cityObj.name + '+' +
    ...
    public onCompanySave() {
        ...
        if (this.commonForm.valid) {
            ...
            this.lat = this.commonForm.value['latitude'];
            this.lng = this.commonForm.value['longitude'];
            this._companyservice.updateCompanyDetails(this.companyId, postData).subscribe((res) => {

                this.companyObs.next(res);
    ...
    public buildGoogleURL(coordinates = null, type = 'lng') {
        ...
        let locationOffice = {};
        if (coordinates != null) {
            locationOffice['lat'] = (type == 'lat') ? coordinates : this.lat;
            locationOffice['long'] = (type == 'lng') ? coordinates : this.lng;

            this.lat = (type == 'lat') ? coordinates : this.lat;
            this.lng = (type == 'lng') ? coordinates : this.lng;
            locationOffice['google_url'] = 'https://www.google.com/maps/embed/v1/place?q=' +
                this.lat + ',' + this.lng + '&key=AIzaSyARNR0nd7PxryzgmXmivhpfWCFvnNBQWT0';

            this.locationOffice$.next(locationOffice);
        } else if (this.lat && this.lng) {

            locationOffice['lat'] = this.lat;
            locationOffice['long'] = this.lng;
            locationOffice['google_url'] = 'https://www.google.com/maps/embed/v1/place?q=' +
                this.lat + ',' + this.lng + '&key=AIzaSyARNR0nd7PxryzgmXmivhpfWCFvnNBQWT0';
            this.locationOffice$.next(locationOffice);
        } else {
